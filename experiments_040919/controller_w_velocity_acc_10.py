...
Simple Controller module that attempts to navigate to randomly generated target locations
    ...
    Use paddle velocity and acceleration as input to the reinforcement learning algo as well
...
## 1. (where we are - where we need to go), 
## 2. (paddle center last frame - paddle center this frame) 
## 3. vel - prev_vel
D = 3 
n_actions = 3
...
if resume:
    ...
    model = {}
    model['W1'] = np.random.randn(H,D) / np.sqrt(D) / 10 # "Xavier" initialization
    model['W2'] = np.random.randn(H,n_actions) / np.sqrt(H) / 10
...
while(episode_number < 2000):
    ...
    if paddle_y != -1 and prev_paddle_y != -1:
        ...
        acc = vel - prev_vel
        x = np.array([target_loc - paddle_y, vel, acc])
    else:
        vel = 0
        acc = 0
        x = np.zeros(D)
    ...
    if done: # an episode finished
        ...
        if episode_number % 100 == 0:
            pickle.dump(model, open('models/control_save_acc_h'+ str(H) +'_' + str(save_counter) + '.p', 'wb'))
            save_counter +=1
