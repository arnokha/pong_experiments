...
# Input dimensionality: 2 object positions for each paddle: (horizontal position, top position, bottom position)
#                       2 object positions for ball (x,y), and the difference of frames in the x and y locations 
#                       of the ball
D = 2 + 2 + 2
...
def get_object_positions(img, display_message=False):
    ...
    return [paddle_1_x, paddle_1_top, paddle_1_bot, 
            paddle_2_x, paddle_2_top, paddle_2_bot, 
...
## Take a few steps into the game and show the object positions (for ipynb)
if plotting:
    for i in range(10):
        observation, reward, done, info = env.step(action_nop)
    poss = get_object_positions(prepro(observation), display_message=True)

    if len(poss) != 0:
        plt.subplot(141)
        plt.imshow(prepro(observation)) # full preprocessed observation
        plt.subplot(142)
        plt.imshow(prepro(observation)[poss[1]:poss[2],poss[0]-4:poss[0]]) # paddle 1
        plt.subplot(143)
        plt.imshow(prepro(observation)[poss[4]:poss[5],poss[3]:poss[3]+4]) # paddle 2
        plt.subplot(144)
        plt.imshow(prepro(observation)[poss[7]:poss[7]+3,poss[6]-1:poss[6]+3]) # ball
    else:
        plt.imshow(prepro(observation)) # full preprocessed observation
...
while True:
    ...
    if prev_x is not None and len(prev_positions) != 0 and len(positions) != 0:
        ...
        x = np.array(positions[:-4] + [dof_ball_x, dof_ball_y])
    else:
        x = np.zeros(D)
    ...
    if done: # an episode finished
        ...
        if episode_number % 200 == 0:
            pickle.dump(model, open('models/absolute_no_extra_save_h'+ str(H) +'_' + str(save_counter) + '.p', 'wb'))
            save_counter +=1
