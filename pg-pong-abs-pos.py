...
# Input dimensionality: 3 object positions for each paddle: (horizontal position, top position, bottom position)
...
while(episode_number < 1500):
    ...
    if prev_x is not None and len(prev_positions) != 0 and len(positions) != 0:
        ...
        x = np.array(positions + [dof_ball_x, dof_ball_y])
    else:
        x = np.zeros(D)
    ...
    if done: # an episode finished
        ...
        if episode_number % 200 == 0:
            ...
            pickle.dump(model, open('models/absolute_save_h'+ str(H) +'_' + str(save_counter) + '.p', 'wb'))
            save_counter +=1
