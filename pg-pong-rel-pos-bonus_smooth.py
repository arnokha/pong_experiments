...
def curriculum_reward(paddle_to_ball_x_distance, avg_y_dist, dof_x_ball):
    if paddle_to_ball_x_distance <= 0 or paddle_to_ball_x_distance > 30 or dof_x_ball <= 0:
        return 0
    scale = 10 / (paddle_to_ball_x_distance) # the closer the ball is to the paddle, the more the reward scale
    #print("Ball distance from paddle (x) is " + str(paddle_to_ball_x_distance) + ", scale is " + str(scale))
    raw_reward = 1 / ((avg_y_dist)**2 + 1) # The smaller the distance between the ball and paddle center, the greater the reward
    #return raw_reward - 0.003
    #print("Ball distance from paddle center (y) is " + str(avg_y_dist) + ", raw r is " + str(raw_reward))
    #return scale * raw_reward - 1
    reward = (20 * scale * raw_reward) - (0.01 * scale)
    #print(reward)
    #print()
    if scale > 5:
        print("Ball distance from paddle (x) is " + str(paddle_to_ball_x_distance) + ", scale is " + str(scale))
...
while(episode_number < 1500):
    ...
    if prev_x is not None and len(prev_positions) != 0 and len(positions) != 0:
        ...
        #if positions[3] > -1 and dof_ball_x > 0:
        reward = curriculum_reward(positions[3], (np.abs(positions[4]) + np.abs(positions[5]/2)), dof_ball_x)
    ...
    observation, real_reward, done, info = env.step(action)
    reward_sum += reward
    ...
    if done: # an episode finished
        ...
        if episode_number % 200 == 0:
            #print('resetting env. episode reward total was ' + str(reward_sum) + '. running mean: ' + str(running_reward))
            pickle.dump(model, open('models/relative_smooth_bonus_save_h'+ str(H) +'_' + str(save_counter) + '.p', 'wb'))
            save_counter +=1
    ...
    if real_reward != 0:
        ...
        print(episode_number, real_reward)
